...
**Amendment — Tier 3 (Jaccard) removed (2026-05-18):** During Phase 5b implementation and testing, Tier 3 was found to act as a "magnet": the single-best-match algorithm at a 0.3 threshold caused a corrected transaction to pull a large number of unrelated transactions into the same category, because the corpus of past categorized transactions shares common tokens (e.g. city names, short words) across many merchants. The proposed fix of requiring a minimum corpus size of N ≥ 3 before activating Tier 3 was rejected on the grounds that it merely delays the problem — once the corpus reaches N, the same magnet behavior resumes. The root issue is that single-best-match with a low threshold is not robust enough without much higher corpus quality or a more discriminative similarity metric. **Resolution:** Tier 3 is removed from the production pipeline. `get_categorized_descriptions` is deleted. `make_categorizer_v2` now accepts three arguments (`user_id`, `keywords`, `aliases`) and applies Tiers 1, 2, and 4 only. The factory signature change is backward-incompatible; all call sites were updated. Tier 6 (local embeddings) remains the documented path if Tier 2 alias coverage proves insufficient after an extended period of user correction history.
