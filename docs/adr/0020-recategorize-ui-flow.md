...
**Amendment1 — edit form copy updated to reflect alias-always-saved behaviour (2026-05-19):** ADR-0018 was amended so that a merchant alias is always written on recategorization, regardless of whether the user checks "apply forward." The original checkbox label ("Apply this rule going forward") implied that nothing would be remembered without it, which was no longer true. Resolution: the checkbox is relabelled "Also save a keyword pattern" and prefaced by a note explaining that the merchant is remembered automatically. The keyword input label changes from "Keyword" to "Keyword pattern" and its hint is updated to guide users toward trimming the description to the core merchant name. No route or service changes — this is a template-only amendment.
**Amendment2 — `?next=` filter state preservation (2026-05-19):** The friction was confirmed: users filtering to uncategorized-only and correcting transactions in sequence were returned to the unfiltered history after every save, requiring them to re-apply the filter each time. Resolution: the history template now includes the current page URL as a `next` query parameter on each Edit link (`url_for('transactions.edit', id=txn.id, next=request.url)`). The GET handler reads `next` from query args and passes it to the template; the form embeds it as a hidden input. The POST handler reads `next` from the form, strips scheme and host via `urlparse` to prevent open redirects, and redirects to the resulting path+query on success (or falls back to the unfiltered history if `next` is absent or unsafe). The "filter state lost" bullet in Consequences is resolved by this amendment.
