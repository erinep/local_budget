...
target in at least one of the look-back months are included.
...
class BudgetTrendCell:
    ...
    pct_used: float    # 0.0 when no_data
    actual: float
...
class BudgetTrendVM:
    title: str
    month_labels: list[str]       # e.g. ['Nov 2025', 'Dec 2025', ...]
...
def build_budget_trend(user_id: str, period_months: int = 6) -> BudgetTrendVM:
    ...
    # Fetch budget progress for each month; collect all budgeted category names
    monthly_progress: list[dict[str, object]] = []
    budgeted_categories: dict[str, None] = {}  # ordered insertion preserves sort
    ...
    for y, m in months:
        ...
        for row in rows:
            if row.target <= 0:
                continue  # exclude unbudgeted-but-spent entries
