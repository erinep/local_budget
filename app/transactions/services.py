...
_PROV_RE = re.compile(
    r'\s+[A-Z]{2,}' r'\s+(' + '|'.join(sorted(_PROV_CODES)) + r')\s*$'
)


def normalize_description(desc: str) -> str:
    """Normalize a transaction description to a canonical merchant name.

    Strips processor prefixes, transaction IDs, store numbers, phone numbers,
    trailing URLs, and city/province suffixes. Returns uppercase.

    Pure function. No database access. ADR-0028 Tier 1.

    Limitation: location words with no preceding store number are not stripped
    without a locations list. The / rule requires at least one digit in the
    token to avoid stripping legitimate slash-separated merchant names.
    """
    s = str(desc).strip()
    ...
    # When a short booking code precedes * and the token after * is a domain, use the domain
    s = re.sub(r'(?i)^[A-Z]{2,5}\*(\S+\.\S+).*$', r'\1', s)

    # Transaction ID after * (alphanumeric-only token — not a domain)
    s = re.sub(r'\*.*$', '', s)

    # Alphanumeric code after / — digit required to avoid stripping real words
    s = re.sub(r'/(?=[A-Z0-9]*\d)[A-Z0-9]{3,}\S*.*$', '', s, flags=re.IGNORECASE)

    # Leading store number (e.g. #243 SPORT CHEK)
    s = re.sub(r'^#\d+\s*', '', s)

    # Store number after merchant name (#1234)
    s = re.sub(r'\s*#\d+.*$', '', s)

    # Trailing phone number
    s = re.sub(r'\s+\d{3}[.\-]\d{3}[.\-]\d{4}.*$', '', s)

    # Trailing URL appearing after merchant name
    s = re.sub(r'(?i)\s+WWW\.\S+', '', s)

    # Store number and everything after it (number + city)
    s = re.sub(r'\s+\d{3,}.*$', '', s)

    # City + province/state suffix — only strip known two-letter codes
    s = _PROV_RE.sub('', s)
...
def make_categorizer_v2(
    ...
    # Sorted longest-first so more-specific aliases win prefix matches.
    _sorted_aliases = sorted(alias_map.items(), key=lambda kv: len(kv[0]), reverse=True)
    ...
    def categorize(desc: str) -> str:
        ...
        # Prefix match — handles trailing city words that normalization can't strip.
        for alias, cat in _sorted_aliases:
